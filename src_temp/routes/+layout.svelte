<script>
	...
	import { ldapUserSignIn, userSignIn, getUserByToken, userSignUp } from '$lib/apis/auths';
	...
	onMount(async () => {
		...
		if (backendConfig) {
			...
			if ($config) {
				...
				let apisecret = querystringValue('apisecret');
				let token = querystringValue('token');
				let clientId = 'Ai';
				let sessionUser = null;
				if (apisecret && token) {
					let user = await getUserByToken(clientId, apisecret, token).catch((error) => {
						toast.error(`${error}`);
						return null;
					});
				...
				function decrypt(encryptedVal) {
					let keyHex = CryptoJS.enc.Utf8.parse('qkjes873l308nmms');
					let ivHex = CryptoJS.enc.Utf8.parse('nmgkszhfwpt66888');
				...
				let signtoken = querystringValue('signtoken');
				if (signtoken) {
